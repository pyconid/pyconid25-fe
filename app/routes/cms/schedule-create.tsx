...
import { postSchedule } from "~/api/endpoint/.server/schedule";
// import { postSpeaker } from "~/api/endpoint/.server/speaker";
import { roomListResponse } from "~/api/schema/room";
import { scheduleCreateRequestSchema } from "~/api/schema/schedule";
...
import type { Route } from "./+types/schedule-create";
...
export const action = async ({ request }: Route.ActionArgs) => {
	...
	const {
		...
		error,
		success,
	} = scheduleCreateRequestSchema.safeParse(json);
	if (!success) {
	...
	const res = await postSchedule({ request, body: validatedJson });
	...
	if (!res.ok) {
		console.error("Failed to create schedule:", res.statusText);
		return {
			clientError: null,
